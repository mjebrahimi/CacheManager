...
namespace CacheManager.RedisAsync.Tests
{
    ...
    public class CacheManagerRegionTests
    {
        ...
        public async Task CacheManager_Region_AddItems_UseSameKeys<T>(T cache)
            ...
            using (cache)
            {
                ...
                foreach (var item in keys)
                {
                    ...
                    foreach (var otherRegion in otherRegions)
                    {
                        ...
                        var val = await cache.GetAsync(key, otherRegion);
                        val.Should()
        ...
        [ClassData(typeof(TestCacheManagers))]
        [Trait("category", "Unreliable")]
        public async Task CacheManager_Region_AddItems_UseDifferentKeys<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                foreach (var item in keys)
                {
                    ...
                    foreach (var otherRegion in otherRegions)
                    {
                        (await cache.GetAsync(key, otherRegion)).Should().Be(null, "the cache should not find the item in other regions");
                    }
                }
            }
        }
            ...
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                try
                {
                    ...
                    await cache.ClearRegionAsync(clearedRegion);

                    await cache.AddAsync("SomeNewItem", "Should be added to the region and the region should be re added to the cache.", clearedRegion);
        ...
        [Theory]
        [ClassData(typeof(TestCacheManagers))]
        public async Task CacheManager_Region_Put_ClearRegion<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                await cache.PutAsync(key, "put value", region);

                (await cache.GetAsync<string>(key, region)).Should().NotBeNull();

                await cache.ClearRegionAsync(region);

                (await cache.GetAsync(key, region)).Should().BeNull(cache.ToString());
            }
        }
        ...
        [ClassData(typeof(TestCacheManagers))]
        [Trait("category", "Unreliable")]
        public async Task CacheManager_Region_RemoveItem_RandomRemoveSomeItems<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    // remove the item
                    await cache.RemoveAsync(key, region);
        ...
        private static Task AddRegionData(ICache<object> cache, int numItems, int numRegions, bool sameKey, out List<Tuple<string, string, string>> keys, out List<string> regions)
        {
            async Task<Tuple<List<Tuple<string, string, string>>, List<string>>> LocalFunc()
            {
                var outKeys = new List<Tuple<string, string, string>>();
                var outRegions = new List<string>();
                ...
                for (var r = 0; r < numRegions; r++)
                {
                    ...
                    for (var i = 0; i < numItems; i++)
                    {
                        ...
                        if (!await cache.AddAsync(key: key, value: value, region: region))
                        {
                            throw new InvalidOperationException("Adding key " + key + ":" + value + " didn't work. For cache\n" + cache.ToString());
                        }

                        outKeys.Add(new Tuple<string, string, string>(region, key, value));
                    }

                    outRegions.Add(region);
                }

                return Tuple.Create(outKeys, outRegions);
            }
