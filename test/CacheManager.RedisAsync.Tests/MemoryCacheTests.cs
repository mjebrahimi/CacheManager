        ...
        public void Dictionary_ExpiredRacecondition()
        {
            ...
            for (var i = 0; i < 100; i++)
            {
                Func<Task> act = async () =>
                {
                    try
                    {
                        var val = await _cache.GetAsync("some_key");
                        if (val == null)
                        {
                            await _cache.PutAsync("some_key", "value");
                        }
                    }
                ...
                Action acc = () => act().GetAwaiter().GetResult();

                Parallel.Invoke(Enumerable.Repeat(acc, 1000).ToArray());
            }
