...
namespace CacheManager.RedisAsync.Tests
{
    ...
    public class SerializerTests
    {
        ...
        [Fact]
        public async Task ProtoBufSerializer_FullAddGet()
        {
            using (var cache = TestManagers.CreateRedisCache(serializer: Serializer.Proto))
            {
                ...
                Func<Task> actSet = () =>
                {
                    return cache.AddAsync(key, pocco);
                };

                // assert
                actSet.Should().NotThrow();
                (await cache.GetAsync<SerializerPoccoSerializable>(key)).Should().BeEquivalentTo(pocco);
            }
        }
        ...
        public async Task BondBinarySerializer_FullAddGet()
        {
        ...
        public async Task Serializer_FullAddGet<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act
                Func<Task<bool>> actSet = () => cache.AddAsync(key, pocco);

                // assert
                (await actSet()).Should().BeTrue("Should add the key");
                cache.GetAsync<SerializerPoccoSerializable>(key).Should().BeEquivalentTo(pocco);
            }
        }

        private async Task FullAddGetWithSerializer(Serializer serializer)
        {
