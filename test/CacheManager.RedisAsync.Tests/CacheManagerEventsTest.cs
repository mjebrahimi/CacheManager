    ...
    public class CacheManagerEventsTest
    {
        ...
        public class LongRunningEventTestBase
        {
                ...
                bool expectValue = true)
            {
                ...
                if (useRegion == null)
                {
                    await cache.AddAsync(useKey, "value");
                    (await cache.GetAsync(useKey)).Should().NotBeNull();
                }
                else
                {
                    await cache.AddAsync(useKey, "value", useRegion);
                    (await cache.GetAsync(useKey, useRegion)).Should().NotBeNull();
                }
                ...
                while (count < 30 && !triggered)
                {
                    if (runGetWhileWaiting)
                    {
                        if (useRegion == null)
                        {
                            foreach (var p in cache.CacheHandles.ToList())
                            {
                                await p.GetAsync(useKey);
                            }
                        }
                        else
                        {
                            foreach (var p in cache.CacheHandles.ToList())
                            {
                                await p.GetAsync(useKey, useRegion);
                            }
                        }
                    }
                ...
                if (endGetShouldBeNull)
                {
                    if (useRegion == null)
                    {
                        (await cache.GetAsync(useKey)).Should().BeNull();
                    }
                    else
                    {
                        (await cache.GetAsync(useKey, useRegion)).Should().BeNull();
                    }
                }
        ...
        [ReplaceCulture]
        public async Task Events_OnGet<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                var resultWithRegion = await cache.GetAsync(key1, region1);
        ...
        [ClassData(typeof(TestCacheManagers))]
        [ReplaceCulture]
        public async Task Events_OnGetWithRegion<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                var data = new EventCallbackData();
                cache.OnGet += (sender, args) => data.AddCall(args, key1);
                await cache.AddAsync(key1, "something", region1);

                // act get without region, should not return anything and should not trigger the event
                var resultWithoutRegion = await cache.GetAsync(key1);
                var result = await cache.GetAsync(key1, region1);
        ...
        [ClassData(typeof(TestCacheManagers))]
        [ReplaceCulture]
        public async Task Events_OnGetMiss<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act
                var result = await cache.GetAsync(key1);
                var resultWithRegion = await cache.GetAsync(key1, region);
        ...
        [ClassData(typeof(TestCacheManagers))]
        [ReplaceCulture]
        public async Task Events_OnGetManyHandles<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
        ...
        public async Task Events_OnRemoveExternal_Redis_UpHandling()
        {
            ...
            await RetryWithCondition(
                ...
                async () =>
                {
                    ...
                    await WaitUntilCancel(async (source) =>
                    {
                        ...
                        (await cache.AddAsync(key, 1234)).Should().BeTrue();

                        var x = await cache.GetAsync(key);
            ...
            cache.CacheHandles.First().GetAsync(key).Should().BeNull();
        }
        ...
        public async Task Events_OnRemoveExternal_Redis_NoneHandling()
        {
            ...
            (await cache.CacheHandles.First().GetAsync(key)).Should().Be(1234);
        }
        ...
        [Trait("category", "Unreliable")]
        [ReplaceCulture]
        public async Task Events_OnRemoveMany<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                await cache.AddAsync(key1, "something", region1);
                await cache.AddAsync(key2, "something", region2);

                // act get without region, should not return anything and should not trigger the event
                var r1 = await cache.RemoveAsync(key1);              // false
                var r2 = await cache.RemoveAsync(key1, region1);    // true
                var r3 = await cache.RemoveAsync(key2, Guid.NewGuid().ToString());   // false
                var r4 = await cache.RemoveAsync(key2, region2);   // true
        ...
        [Trait("category", "Unreliable")]
        [ReplaceCulture]
        public async Task Events_OnAddMany<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act get without region, should not return anything and should not trigger the event
                var r1 = await cache.AddAsync(key1, "something", region1);  // true
                var r2 = await cache.AddAsync(key2, "something", region2); // true
                var r3 = await cache.AddAsync(key1, "something", region1);  // false
                var r4 = await cache.AddAsync(key2, "something", region2); // false
                var r5 = await cache.AddAsync(key1, "something");            // true
                var r6 = await cache.AddAsync(key1, "something");            // false
        ...
        [Trait("category", "Unreliable")]
        [ReplaceCulture]
        public async Task Events_OnPutMany<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act get without region, should not return anything and should not trigger the event
                await cache.PutAsync(key1, "something", region1);
                await cache.PutAsync(key2, "something", region2);
                await cache.PutAsync(key1, "something", region1);
        ...
        [Trait("category", "Unreliable")]
        [ReplaceCulture]
        public async Task Events_OnUpdate<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act get without region, should not return anything and should not trigger the event
                (await cache.AddAsync(key1, 1, region1)).Should().BeTrue("add key1 to region");
                (await cache.AddAsync(key2, 1, region2)).Should().BeTrue("add key2 to region2");
                (await cache.AddAsync(key1, 1)).Should().BeTrue("add key1");

                object val;
                (await cache.TryUpdateAsync(key1, region1, async o => ((int)o) + 1, out val)).Should().BeTrue();
                (await cache.TryUpdateAsync(key2, region2, async o => ((int)o) + 1, out val)).Should().BeTrue();
                (await cache.TryUpdateAsync(key1, async o => ((int)o) + 1, out val)).Should().BeTrue();
        ...
        [Trait("category", "Unreliable")]
        [ReplaceCulture]
        public async Task Events_OnClearRegion<T>(T cache)
            where T : ICacheManager<object>
        {
            using (cache)
            {
                ...
                // act get without region, should not return anything and should not trigger the event
                await cache.ClearRegionAsync(region1);
                await cache.ClearRegionAsync(region2);
        ...
        [Fact]
        [ReplaceCulture]
        public async Task Events_OnClear()
        {
        ...
        private class CustomRemoveEventTestHandle : BaseCacheHandle<string>
        {
            ...
            protected override async Task<bool> AddInternalPreparedAsync(CacheItem<string> item)
            {
                throw new NotImplementedException();
            }

            protected override async Task<CacheItem<string>> GetCacheItemInternalAsync(string key)
            {
                throw new NotImplementedException();
            }

            protected override async Task<CacheItem<string>> GetCacheItemInternalAsync(string key, string region)
            {
                throw new NotImplementedException();
            }

            protected override async Task PutInternalPreparedAsync(CacheItem<string> item)
            {
                throw new NotImplementedException();
            }
