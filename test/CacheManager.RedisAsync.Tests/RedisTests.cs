    ...
    public class RedisTests
    {
        ...
        public async Task Redis_ValidateVersion_AddPutGetUpdate()
        {
            ...
            using (cache)
            {
                ...
                await cache.UpdateAsync(key, async r => { r.Something = "new text"; return r; });
                await Task.Delay(10);

                version = (int)multi.GetDatabase(0).HashGet(key, "version");
                version.Should().Be(3);
                (await cache.GetAsync(key)).Something.Should().Be("new text");
            }
        }
        ...
        [Trait("category", "Redis")]
        [Trait("category", "Unreliable")]
        public async Task Redis_UseExistingConnection()
        {
            ...
            {
                await cache.AddAsync(Guid.NewGuid().ToString(), 12345);
            }
        }
        ...
        public async Task Redis_BackplaneEvents_AddWithRegion()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                async (cacheA) =>
                {
                    await cacheA.AddAsync(key, key, region);
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_InMemory_AddWithRegion()
        {
            ...
            await TestBackplaneEventInMemory<CacheActionEventArgs>(
                ...
                async (cacheA, cacheB) =>
                {
                    ...
                    await cacheB.AddAsync(key, key, region);
                    await cacheA.AddAsync(key, key, region);
                },
                (cacheA, args) =>
                {
        ...
        public async Task Redis_BackplaneEvents_Put()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                {
                    await cacheA.AddAsync(key, key);
                    await cacheA.PutAsync(key, "new val");
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_PutWithRegion()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                {
                    await cacheA.AddAsync(key, key, region);
                    await cacheA.PutAsync(key, "new val", region);
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_Remove()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                async (cacheA) =>
                {
                    (await cacheA.AddAsync(key, key)).Should().BeTrue();
                    (await cacheA.RemoveAsync(key)).Should().BeTrue();
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_Remove_WithRegion()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                {
                    (await cacheA.AddAsync(key, key)).Should().BeTrue();
                    (await cacheA.AddAsync(key, key, region)).Should().BeTrue();
                    (await cacheA.RemoveAsync(key, region)).Should().BeTrue();
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_InMemory_Remove_WithRegion()
        {
            ...
            await TestBackplaneEventInMemory<CacheActionEventArgs>(
                ...
                async (cacheA, cacheB) =>
                {
                    ...
                    (await cacheB.AddAsync(key, key, region)).Should().BeTrue();
        ...
        public async Task Redis_BackplaneEvents_Update()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                {
                    await cacheA.AddAsync(key, key);
                    (await cacheA.UpdateAsync(key, async v => newValue)).Should().Be(newValue);
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_Update_WithgRegion()
        {
            ...
            await TestBackplaneEventDistributed<CacheActionEventArgs>(
                ...
                {
                    await cacheA.AddAsync(key, key, region);
                    (await cacheA.UpdateAsync(key, region, async v => newValue)).Should().Be(newValue);
                },
                (cacheA, args) =>
                {
                    args.Key.Should().Be(key);
        ...
        public async Task Redis_BackplaneEvents_Clear()
        {
            ...
            await TestBackplaneEventDistributed<CacheClearEventArgs>(
                ...
                {
                    args.Origin.Should().Be(CacheActionEventArgOrigin.Local);
                    (await cacheA.GetAsync(key)).Should().BeNull();
                },
                async (cacheB, args) =>
                {
                    args.Origin.Should().Be(CacheActionEventArgOrigin.Remote);
                    (await cacheB.GetAsync(key)).Should().BeNull();
                });
        }
        ...
        public async Task Redis_BackplaneEvents_ClearRegion()
        {
            ...
            await TestBackplaneEventDistributed<CacheClearRegionEventArgs>(
                ...
                async  (cacheA) =>
                {
                    await cacheA.AddAsync(key, key);
                    await cacheA.AddAsync(key, key, region);
                    await cacheA.ClearRegionAsync(region);
                },
                async  (cacheA, args) =>
                {
                    args.Origin.Should().Be(CacheActionEventArgOrigin.Local);
                    (await cacheA.GetAsync(key)).Should().NotBeNull();
                    (await cacheA.GetAsync(key, region)).Should().BeNull();
                },
                async (cacheB, args) =>
                {
                    args.Origin.Should().Be(CacheActionEventArgOrigin.Remote);
                    (await cacheB.GetAsync(key)).Should().NotBeNull();
                    (await cacheB.GetAsync(key, region)).Should().BeNull();
                });
        }
        ...
        public async Task Redis_Multiple_PubSub_Change()
        {
            ...
            await RunMultipleCaches(
                async (cacheA, cacheB) =>
                {
                    await cacheA.PutAsync(item);
                    (await cacheA.GetAsync(item.Key)).Should().Be("something");
                    await Task.Delay(10);
                    var value = await cacheB.GetAsync(item.Key);
                    value.Should().Be(item.Value, cacheB.ToString());
                    await cacheB.PutAsync(item.Key, "new value");
                },
                async (cache) =>
                {
                    int tries = 0;
                    ...
                    do
                    {
                        ...
                        value = await cache.GetAsync(item.Key);
                    }
        ...
        public async Task Redis_Multiple_PubSub_Clear()
        {
            ...
            await RedisTests.RunMultipleCaches(
                async (cacheA, cacheB) =>
                {
                    await cacheA.AddAsync(item);
                    (await cacheB.GetAsync(item.Key)).Should().Be(item.Value);
                    await cacheB.ClearAsync();
                    await Task.Delay(0);
                },
                async (cache) =>
                {
                    (await cache.GetAsync(item.Key)).Should().BeNull();
                    await Task.Delay(0);
                },
                2,
        ...
        public async Task Redis_Multiple_PubSub_ClearRegion()
        {
            ...
            await RedisTests.RunMultipleCaches(
                ...
                {
                    await cacheA.AddAsync(item);
                    (await cacheB.GetAsync(item.Key, item.Region)).Should().Be(item.Value);
                    await cacheB.ClearRegionAsync(item.Region);
                    await Task.Delay(0);
                },
                async (cache) =>
                {
                    (await cache.GetAsync(item.Key, item.Region)).Should().BeNull();
                    await Task.Delay(0);
                },
                2,
        ...
        public async Task Redis_Multiple_PubSub_Remove()
        {
            ...
            await RedisTests.RunMultipleCaches(
                async (cacheA, cacheB) =>
                {
                    await cacheA.AddAsync(item);
                    (await cacheB.GetAsync(item.Key)).Should().Be(item.Value);
                    await cacheB.RemoveAsync(item.Key);
                    await Task.Delay(10);
                },
                async (cache) =>
                {
                    ...
                    do
                    {
                        tries++;
                        await Task.Delay(100);
                        value = await cache.GetCacheItemAsync(item.Key);
                    }
        ...
        public void Redis_Verify_NoCredentialsLoggedOrThrown()
        {
            ...
            Func<Task> act = () => new BaseCacheManager<string>(cfg).PutAsync("key", "value");
        ...
        public async Task Redis_NoRaceCondition_WithUpdate()
        {
            ...
            }))
            {
                ...
                await cache.AddAsync(key, new RaceConditionTestElement() { Counter = 0 });
                int numThreads = 5;
                int iterations = 10;
        ...
        public void Redis_LoadWithRedisBackplane_FromAppConfig()
        {
            ...
            Func<Task> count = async () => { var x = await handle.CountAsync(); };
        ...
        [Fact]
        [Trait("category", "Redis")]
        public async Task Redis_ValueConverter_CacheTypeConversion_Poco()
        {
            ...
            using (cache)
            {
                ...
                var value = new Poco() { Id = 23, Something = "§asdad" };
                await cache.AddAsync(key, value);
                var result =  (Poco)await cache.GetAsync(key);
        ...
        [Fact]
        [Trait("category", "Redis")]
        public async Task Redis_ValueConverter_Poco_Update()
        {
            ...
            using (cache)
            {
                ...
                Func<Task<bool>> act = () => cache.TryUpdateAsync(key, region, async (o) => newValue, out resultValue);
        ...
        public async Task Redis_ValueConverter_ValidateValuesTypesNotUsingSerializer<T>(T value)
        {
